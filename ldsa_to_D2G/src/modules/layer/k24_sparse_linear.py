...
Adapted from Kaleidoscope K24 implementation for LDSA.
Key differences:
- Uses n_agents instead of n_unit_types for MPE environments
...
class SemiStructuredLinear24(nn.Linear):
    """
    Custom Linear layer with 2:4 semi-structured sparsity for LDSA agents.
    ...
    Args:
        n_agents: Number of agents (for heterogeneous masks)
    ...
    def reset_hetero_alpha(self, reset_mask=None):
        ...
        Args:
            reset_mask: [n_agents, out_features, in_features] boolean mask
                ...
                If float, reset randomly with that probability
        """
        with th.no_grad():
            ...
            if reset_mask is None:
                ...
                # Float probability (e.g., 0.1)
                mask = th.rand_like(self.hetero_alpha) < reset_mask
            else:
                # Tensor mask, use directly
                mask = reset_mask

            # Ensure mask and alpha are on same device
            if isinstance(mask, th.Tensor):
                mask = mask.to(self.hetero_alpha.device)

            # 2. Generate new random initial values
