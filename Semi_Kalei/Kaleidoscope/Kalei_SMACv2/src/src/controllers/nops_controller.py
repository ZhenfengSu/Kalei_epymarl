...
class NoPSMAC:
    """
    No-Parameter-Sharing Multi-Agent Controller.
    ...
    def select_actions(self, ep_batch, t_ep, t_env, bs=slice(None), test_mode=False):
        """Select actions for the selected batch elements"""
    ...
    def forward(self, ep_batch, t, test_mode=False):
        """Forward pass to compute agent outputs"""
    ...
    def init_hidden(self, batch_size):
        ...
        if self.hidden_states is not None:
            self.hidden_states = self.hidden_states.expand(batch_size, -1, -1)  # (b, a, h)

    def parameters(self):
        """Return all parameters of the agent networks"""
    ...
    def _build_inputs(self, batch, t):
        ...
        # Note: NoPS doesn't need agent_id since each agent has its own network
    ...
    def _get_input_shape(self, scheme):
        """Get the input shape for agents"""
