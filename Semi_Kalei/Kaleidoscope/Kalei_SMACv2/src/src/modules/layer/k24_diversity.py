"""
K-2:4 Pattern Orthogonality Diversity Loss (SMACv2)

Implements Module C: Pattern Orthogonality (Heterogeneity Enforcement)

This module enforces unit type diversity by encouraging different unit types to select
...
class PatternOrthogonalityLoss(nn.Module):
    ...
    Encourages unit types to select different 2:4 sparse patterns by minimizing
    ...
    Where:
        pi_i,g: Softmax probability distribution over 6 patterns for unit type i, group g
    ...
    def __init__(self, n_unit_types, normalize=True):
        super().__init__()
    ...
    def forward(self, pattern_probs_list):
        """
        Compute pattern orthogonality loss across all unit types.

        Args:
            pattern_probs_list: List of [n_unit_types, ...] tensors
                Each tensor contains pattern probabilities for one layer
                Shape after reshape: [n_unit_types, n_groups, 6]
        ...
        for pattern_probs in pattern_probs_list:
            ...
            # pattern_probs: [batch, n_unit_types, n_groups, 6] or [n_unit_types, n_groups, 6]
            # Average over batch dimension if present
            if pattern_probs.dim() == 4:
                pattern_probs = pattern_probs.mean(dim=0)  # [n_unit_types, n_groups, 6]

            n_types_in_batch = pattern_probs.shape[0]
            n_groups = pattern_probs.shape[1]

            # Compute pairwise orthogonality loss
            # For each group, compute dot product between all type pairs
            group_losses = []

            for g in range(n_groups):
                # pattern_probs[:, g, :]: [n_unit_types, 6]
                pi = pattern_probs[:, g, :]  # [n_unit_types, 6]

                # Compute all pairwise dot products
                # [n_unit_types, 6] @ [6, n_unit_types] -> [n_unit_types, n_unit_types]
        ...
        # Normalize by number of type pairs
        if self.normalize:
            n_pairs = self.n_unit_types * (self.n_unit_types - 1) / 2
...
class K24DiversityManager:
    ...
    def __init__(self, n_unit_types, base_div_coef=0.1, deque_len=100):
        self.n_unit_types = n_unit_types
        ...
        self.diversity_loss = PatternOrthogonalityLoss(n_unit_types, normalize=True)
