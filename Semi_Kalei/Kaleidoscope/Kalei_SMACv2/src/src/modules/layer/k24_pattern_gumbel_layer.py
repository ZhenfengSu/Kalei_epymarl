"""
K-2:4 Pattern-based Gumbel-Softmax Layer for Semi-structured Sparsity (SMACv2)
...
Adapted from MPE implementation for SMACv2 environments.
Key differences:
- Uses n_unit_types instead of n_agents for SMACv2
- Compatible with SMACv2's unit-type-based agent system
"""
...
class SemiStructuredLinear24(nn.Linear):
    """
    Custom Linear layer with 2:4 semi-structured sparsity for each unit type (SMACv2).
    ...
    Args:
        n_unit_types: Number of unit types in SMACv2
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # EMA activation tracker (one per layer, shared across unit types)
        self.ema_tracker = EMAActivationTracker(momentum=ema_momentum)

        # Module A: Heterogeneity coefficients (unit-type-specific)
        # Each unit type has its own alpha to modulate weight importance
        self.hetero_alpha = nn.Parameter(
            th.randn(n_unit_types, self.out_features, self.in_features) * hetero_init_scale
        )
        ...
        self.register_buffer('frozen_mask', th.ones(n_unit_types, self.out_features, self.in_features))
    ...
    def _compute_heterogeneous_scores(self, agent_ids):
        ...
        Returns:
            scores: [batch_size, out_features, in_features]
                Heterogeneous importance scores for each unit type
        """
        ...
        # 3. Unit-type-specific heterogeneity coefficients
    ...
    def forward(self, x, agent_ids):
        ...
        Args:
            ...
            agent_ids: [batch_size] unit type indices for heterogeneous masks
    ...
    def reset_hetero_alpha(self, reset_mask=None):
        ...
        Args:
            reset_mask: [n_unit_types, out_features, in_features] boolean mask
    ...
    def get_sparsity(self):
        ...
        with th.no_grad():
            # Sample from all unit types
            agent_ids = th.arange(self.n_unit_types, device=self.weight.device)
            scores = self._compute_heterogeneous_scores(agent_ids)
    ...
    def freeze_mask(self):
        """
        Freeze the current mask pattern for all unit types.

        This implements the lottery ticket hypothesis "rewind" step:
        - Samples the current mask for all unit types
        ...
        """
        with th.no_grad():
            # Get unit type IDs for all types
            agent_ids = th.arange(self.n_unit_types, device=self.weight.device)

            # Compute scores for all types
            scores = self._compute_heterogeneous_scores(agent_ids)

            # Get current masks (using Gumbel-Softmax)
            masks, _ = self._pattern_gumbel_softmax(scores)

            # Store masks per type [n_unit_types, out_features, in_features]
...
def create_k24_linear(n_unit_types, hidden_dim, **kwargs):
    """Factory function to create SemiStructuredLinear24 layer."""
    return partial(SemiStructuredLinear24, n_unit_types=n_unit_types, hidden_dim=hidden_dim, **kwargs)
