...
class SNP_type_NRNNAgent_1R3(nn.Module):
    """
    SNP (Structured Network Pruning) Agent implementation.
    ...
    def __init__(self, input_shape, args):
        super(SNP_type_NRNNAgent_1R3, self).__init__()
        self.args = args
        ...
        for i, layer_sparsity in enumerate(self.sparsity_ratios):
            self.register_buffer(
                f"mask_{i}",
                th.rand(self.n_agents, args.rnn_hidden_dim) > layer_sparsity,
            )
    ...
    def forward(self, inputs, hidden_state, agent_ids):
        ...
        Args:
            ...
            agent_ids: Agent type IDs [batch_size, n_agents]
