"""
Kaleidoscope Q-Learner with unstructured pruning for agent heterogeneity.

Extends QLearner to add:
- Diversity loss for encouraging heterogeneous sparse masks
- Dynamic mask reset mechanism
- Adaptive diversity coefficient based on loss history
"""
import copy
...
class Kalei_QLearner(QLearner):
    """
    Q-Learner for Kaleidoscope algorithm.

    Implements training with:
    - TD loss for Q-learning
    - Diversity loss for agent heterogeneity
    - Periodic mask resetting for exploration
    """
    ...
    def __init__(self, mac, scheme, logger, args):
        ...
        # Loss history for adaptive diversity coefficient
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        """
        # Enable gradients for mask parameters
        self.mac.agent.set_require_grads(mode=True)

        # Reset masks if interval has passed (but not near end of training)
        if (
        ...
        # Diversity loss for heterogeneity
